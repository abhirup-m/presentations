...
#show heading.where(level: 2): set text(fill: h2Color, font: h2Font, size: 33pt)
...
== Insights on Mottness in Correlated Systems
#v(0.5em)
#text(weight: "bold", size: 1.4em, [ABHIRUP MUKHERJEE])

#text(size: 1.4em, [Postdoctoral Presentation at _Seoul National University_])
#v(0.5em)
#grid(
...
#slide[
  ...
  #let timeline = box(
      ...
      + place(left + horizon, dx: 3em, circle(fill: h2Color))
      + place(center + horizon, circle(fill: h2Color))
      + place(right + horizon, dx: -3em, circle(fill: h2Color))
  )
  #cols(
    [
      #cols(
        [University of Calcutta\ (*B.Sc.* Physics)],
        [IISER Kolkata (*M.Sc.*)],
        [IISER Kolkata (*Ph.D.*)],
        w: (1fr, 0.8fr, 1fr)
      )
...
#slide[
  ...
  #let items = ()
  #for key in ("Mukherjee2023", "Mukherjee2024", "Mukherjee2025") {
  ...
  #uncover(auto)[
    #place(
      top + left,
      dy: -0.3em,
      rect(width: 100%, height: 1.5em, stroke: (left: 0.4em + h2Color, rest: 0em)),
    )

    #place(
      top + left,
      dy: 3.5em,
      rect(width: 100%, height: 2em, stroke: (left: 0.4em + h2Color, rest: 0em)),
    )
  ]
]
...
#slide[
  #title("$T=0$ Scaling Analysis of Impurity Model")
...
#slide[
  #title("Quantum Entanglement Becomes Long-Ranged")
...
#slide[
  ...
  #only(2,3)[
    ...
    #place(
      ...
      dy: 3em,
      bbox(factor: 120%,
        [characterised by ANOMALY associated\ with a GENERALISED SYMMETRY!]
      )
    )
    ]
  ]
...
#slide[
  ...
  #img("urg-scheme.svg", w: 80%)
  #img("urg_ham_full.svg", w: 50%)
