...
\begin{frame}{A Brief Introduction}
	...
	\begin{itemize}
		...
		\item {\bf Numerical computation} of correlation functions, entanglement measures, dynamical correlations (spectral function, self-energy, etc)
		\item Developed {\bf new method} for using impurity models to analyse interacting lattice models
		\item Developed Python and Julia {\bf libraries}
	\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item Which metal is \focus{parent phase} of Mott insulator/ superconductor, \focus{at \(1/2-\)filling}?
...
\begin{frame}{Universal Scaling of Spectral Features}
	\begin{itemize}
		\item Electron Scattering Rate of NFLs \focus{cross Mott-Ioffe-Regel (MIR) bound} (no electron-like excitations!), while FLs are within it.\\[10pt]
...
\end{minipage}
\begin{itemize}
	\item Nodal metal is singular, i.e., has a Mott pole in self-energy at Fermi energy, but is still gapless. Gapless excitations are holons \& doublons.\\[10pt]
	\item Persistent \(\Sigma^{\prime\prime} \sim \omega^{-2}, A \sim \omega^2\) behaviour throughout the PG suggests an HK-type model is \focus{universal} fixed-point theory for Mott metal phase.
\end{itemize}
