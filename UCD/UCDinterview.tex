...
\title{\huge Mott Criticality as the Confinement\\ Transition of a Pseudogap-Mott Metal}
\subtitle{Postdoctoral Presentation at UCD}

\author{\focus{Abhirup Mukherjee}}
...
\begin{itemize}
	\item[\ding{51}] {\bf Mott Criticality as the Confinement Transition of a Pseudogap-Mott Metal}. New J. Phys. 28 013503 (2026)\\[10pt]
...
\subsection{\large New J. Phys. 25 113011 (2026). Abhirup Mukherjee, S R. Hassan, A Mukherjee, N S. Vidhyadhiraja, A Taraphder, S Lal}
...
\begin{frame}{Our Approach, In a Nutshell}
	...
	\hfill
	\begin{minipage}{0.45\textwidth}
	Solve an appropriate \alert{impurity model}:
	\begin{itemize}
		\item Lattice symmetry + Transition
	\end{itemize}~\\
	\focus{Construct lattice} model by applying many-body translation operators (``tiling")
	% \begin{itemize}
	% 	\item Restore discrete translation invariance
	% \end{itemize}~
	% \focus{Reconstructed lattice Hamiltonian:}
	\[H_\text{hop} + \tilde J \sum_{\left< {\bf r}_i, {\bf r}_j\right>}{\bf S}_{{\bf r}_i}\cdot{\bf S}_{{\bf r}_j} - \frac{1}{2}\tilde U\sum_{\bf r}\left(\hat n_{{\bf r} , \uparrow} - \hat n_{{\bf r} , \downarrow}\right)^2
	\]
...
\begin{frame}{Parent State of Mott Insulator: Non-Fermi Liquid Pseudogap}
...
\begin{frame}{Long-ranged Entanglement and Singular Nodal Metal}
\begin{itemize}
	\item Optical Conductivity \(\sigma \sim A(\omega) \tau(\omega)\) shows a \focus{shifted ``Drude" peak}
	\item real-space entanglement undergoes a crossover within the pseudogap from short-ranged to \focus{long-ranged} and \(5-\)partite behaviour\\[10pt]
...
\begin{center}
	...
	\hfill
	\includegraphics[width=0.26\textwidth]{nodalMetal.pdf}
\end{center}
\end{frame}
...
% 	\begin{itemize}
% 		\item  \focus{Studying electronic correlations in MATBG through an exactly solvable model}. The Hatsugai-Kohmoto model is an exactly solvable correlated model displaying non-Fermi liquid and Mott insulating phases. This therefore provides an interesting route to studying the emergence of novel phases in a correlated MATBG system, with {\bf analytical clarity}.\\[20pt]
% 		\item \focus{Exploring violation of Luttinger's theorem in heavy-fermion systems}. Violation of Luttinger's theorem may result from changes in the anomaly structure of the Fermi surface. One can investigate these claims particularly in the context of heavy-fermion systems to write down {\bf topological order parameters} that track phase transitions in such systems.\\[20pt]
% 		\item \focus{Studying effect of electronic correlations in MATBG through a bilayer impurity model}. Following Prof. Bernevig's {\bf topological heavy-fermions approach} towards MATBG, it may be fruitful to look at an impurity model analogue of such a THF model by allowing a Kondo impurity to hybridise with a topologically non-trivial conduction bath.
% 	\end{itemize}
% \end{frame}
%
\begin{frame}{}
...
\section{\Large Backup Slides\vspace*{10pt}}
\subsection{~}
...
\end{minipage}
\end{frame}
\section{\Large Theoretical Techniques}
\subsection{~}
...
\begin{minipage}{0.65\textwidth}
	\begin{itemize}
		\item Provides access to \focus{low energy fixed-point Hamiltonian}
		\item RG Proceeds via decoupling of high energy degrees of freedom via many-body \focus{unitary} transformations 
		\item Transformations account for \focus{many-body scattering processes} into various energy sectors\\[10pt]
		\item Also used other RG methods such as \focus{``Poor Man's scaling"}
	\end{itemize}
\end{minipage}
\end{frame}

\begin{frame}{Exact Diagonalisation and Correlation Functions}
	\vspace{-5pt}
	\begin{itemize}
		\item Experience in working with \focus{fermionic hamiltonians}
		\item Numerical modelling and exact diagonalisation. 
	\end{itemize}

	\vspace{20pt}
	\begin{minipage}{0.32\textwidth}
	\centering
	\focus{Static correlation functions}
	\begin{itemize}
		\item Spin/charge-correlations
		\item Order parameters
	\end{itemize}
	\vspace{0.5em}
	\includegraphics[height=0.6\textwidth]{sdz_77-1500.pdf}
	\end{minipage}
	\begin{minipage}{0.32\textwidth}
	\centering
	\focus{Dynamical responses}
	\begin{itemize}
		\item Spectral functions
		\item Self-energies
	\end{itemize}
	\vspace{0.5em}
	\includegraphics[height=0.6\textwidth]{Ad_zoomin_77-1500.pdf}
	\end{minipage}
	\begin{minipage}{0.32\textwidth}
	\centering
	\focus{Entanglement measures}
	\begin{itemize}
		\item Ent. entropy, mutual info.
		\item Quantum Fisher info.
	\end{itemize}
	\vspace{0.5em}
	\includegraphics[height=0.6\textwidth]{qfi_77-2000.pdf}
	\end{minipage}
	\vfill
	\vspace{15pt}
...
\begin{frame}{Miscellaneous methods}
	\begin{itemize}
		\item \focus{Spectral flow} insertion techniques: Boundary-condition twisting
		\item \focus{Wavefunction} renormalisation calculations
		\item Effective Hamiltonian computation (\focus{renormalised perturbation} theory)
	\end{itemize}
	\vfill
	Interested in learning \focus{new} methods.\\[10pt]
	Have a basic grasp on the theory behind methods like \focus{DMRG, NRG, DMFT}. 
	\vfill
