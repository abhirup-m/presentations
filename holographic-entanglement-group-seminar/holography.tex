...
\begin{frame}{Broad overview of results}
	\begin{itemize}
		\item Entanglement scaling within a 2D system of Dirac electrons can be used to construct a holographic bulk dimension.\\[10pt]
		\item A Fermi surface-changing transition of the electronic system corresponds to a "wormhole geometry" in the bulk.\\[10pt]
		\item The topology of the Fermi surface is dictated by the entanglement structure.
	\end{itemize}
\end{frame}
...
\only<1>{
Dispersion is \focus{linear} in momentum space
\begin{center}
\includegraphics[width=0.8\textwidth]{diracOrigin.pdf}
\end{center}
\begin{itemize}
	\item Describe the \focus{low-energy} theory near gap-closing points
	\item Emerge at boundaries of \focus{topological insulators}
\end{itemize}
}
\only<2>{
	\begin{itemize}
...
\begin{frame}{Measures of entanglement}
	...
	\begin{itemize}
		\item \(S(A) = -\text{Tr}\left[\rho_A \log \rho_A\right] \longrightarrow\) \focus{entanglement entropy} of A\\[10pt]
		\item \(I(A:B) = S(A) + S(B) - S(A \cup B) \longrightarrow\) \focus{mutual information} between \(A\) and \(B\)\\[10pt]
		\item quantifies amount of \focus{information shared} between subsystems
	\end{itemize}
\end{frame}
...
\only<1>{
	...
	\begin{itemize}
		\item \focus{Vanishing} entanglement in momentum space\\[10pt]
		\item Off-diagonal in \(r-\)space \(\mathbf\longrightarrow\) \focus{Fluctuations} exist in real space\\[10pt]
...
\only<2>{
	\begin{itemize}
		\item \(H = \sum_n H_n \implies \rho = \exp{\left(-\beta H\right)} = \otimes_n \rho_n \implies\) no entanglement in \(k_x-\)space
		\item Entanglement reduces to sum over 1D modes: \(S(\left[x_1, x_2\right])  = \sum_n S_n(\left[x_1, x_2\right])\)
	\end{itemize}
...
\begin{frame}{Entanglement in topologically ordered phases}
	...
	These are gapped quantum liquids\\[10pt]
	\begin{itemize}
...
\vspace*{\fill}
{\it {\large \bf Question:} Does this say anything for the cosmological constant?}
}
\only<5>{
