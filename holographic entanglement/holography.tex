...
\begin{frame}{Entanglement of free fermions}
	massless fermion on \(1-\)d line: \(~ ~ ~ \frac{1}{3}\log \left(l/\epsilon\right) \)\\[20pt]

	massive fermions on \(1-\)d line: \(~ ~ ~ \frac{1}{3}\log \left(l/\epsilon\right) - \frac{1}{6}\left( ml \log ml \right)^2 \)\\[20pt]

	massless fermions in higher dims.: \(~ ~ ~ ~ l^{d-1}\log l\)\\[20pt]
...
\begin{frame}{What are we going after?}
	\begin{itemize}
		\item Distribution of entanglement across subsystems\\[20pt]
		\item Emergent space generated by this entanglement \\[20pt]
...
\begin{frame}{The sign of the curvature is topological}
	...
	\begin{itemize}
		...
		\item change in sign of \(z\) might then be a \focus{topological phase transition} in the microscopic theory
	\end{itemize}
\end{frame}
...
\only<1>{
	Define an expansion parameter: \(\theta_z(j) = \frac{1}{\sqrt{1 + v_z^{-2}}}\)\\[20pt]
	can be related to RG flow through \(\beta_z\)\\[20pt]
	related to change in area of flows of \(g_z\)
...
\begin{frame}{Conclusions}
	\begin{itemize}
		\item hierarchy of entanglement, both across scales as well as number of parties\\[20pt]
		...
		\item \(\theta\), \(\tilde\theta\) satisfy "Raychaudhuri-like" equations
	\end{itemize}
\end{frame}
