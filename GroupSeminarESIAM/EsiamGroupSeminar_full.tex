...
\begin{itemize}[<+->]
	\item Replace fluctuations by an \alert{effective mean field} $h(y)$, where \(y\) is some order parameter.
		\only<1>{\[J \sum_i S_i^z S_{i+1}^z \to h \sum_i S_i^z, h=\beta J m\]}\\[10pt]
...
\item Solve the simpler problem at a particular local site by starting with some \alert{guess value \(h_\text{guess}\) for the mean-field}. From the solution, calculate the order parameter \(y_\text{guess}\) corresponding to the guess field.
	\only<3>{\[h=0.01\implies m=0.02 (say)\]}\\[10pt]
\item From the consistency equation, the order parameter gives the next value of the field: \(h = f^{-1}_\text{next}(y_\text{guess})\), leading to an \alert{updated mean-field Hamiltonian}.
	\only<4>{\[h=0.01\implies m=0.02 \implies h_\text{next} = \arctan(0.02)\]}\\[10pt]
...
\only<5>{\[h=0.059\implies h_\text{next} = \arctan(0.0591) \implies h_\text{next} = \arctan(0.0589) \]}
\end{itemize}
...
\[H = H_\text{KE} + V\sum_\sigma\left(c^\dagger_{d\sigma}c_{0\sigma} + \text{h.c.}\right) + \frac{U}{2}\left( \hat n_{d \uparrow} - \hat n_{d \downarrow} \right)^2 + J \vec{S}_d\cdot\vec{S}_0 + U_b\left( \hat n_{0 \uparrow} - \hat n_{0 \downarrow} \right)^2\]
}
...
\footcite{moeller_1995,held_2013}
...
\begin{itemize}
	\nitem For \(|U_b| \lesssim J/4\), central peak and side peaks are \alert{well-separated} \\[20pt]
	\nitem \alert{Integrate out} charge fluctuations through Schrieffer-Wolff transformation\\[10pt]
\[H_\text{eff} = \tilde J \vec{S}_d\cdot\vec{S}_0 - \tilde U_b\left(\hat n_{0 \uparrow} - \hat n_{0 \downarrow}\right)^2 + H_\text{K.E.}\]
\[ \text{RG equation for } \tilde J: ~ \Delta \tilde J \sim \tilde J \left( \tilde J  + 4\tilde U_b \right) \]
\end{itemize}
\end{minipage}
\hspace*{\fill}
\begin{minipage}{0.3\textwidth}
...
\only<2>{
\begin{itemize}
	\nitem \alert{captures the criticality}, and the strong-coupling and local moment phases\\[20pt]
\end{itemize}
...
\begin{itemize}
\only<1>{
	\nitem \alert{Integrate out impurity site} from fixed point Hamiltonian via a single URG transformation\\[20pt]
\nitem Generates additional correlation $U_0$ on zeroth site
\vspace*{\fill}
\begin{center}
	\includegraphics[width=0.6\textwidth]{decouple-impurity.pdf}\\[20pt]
\end{center}
\nitem \(J\) is relevant and the largest scale \(\longrightarrow\) \alert{repulsive correlation}:
\[U_0 + U_b \simeq J^*/8 + \frac{8}{3}{V^*}^2/J^* > 0\]
}
\only<2>{
...
\only<2>{
\vspace*{\fill}
\textbf{Essence of \alert{self-consistency}: Equivalence of impurity and zeroth sites!}
}
\end{frame}
...
\nitem Mott gap appears \alert{discontinuously} after the transition, through a \alert{continuous} sharpening of the central peak.\\[10pt]
\nitem \alert{True} transition believed to occur at \(U_{c2}\).
\end{itemize}
}
\only<2->{
\textbf{\alert{Can be explained} heuristically using the two site spectrum}\\[10pt]
\vspace*{\fill}
\begin{itemize}
...
\only<1>{
\textbf{\alert{Poor Man's scaling} of the effective Kondo model}\\
\small{\it [K. Held, R. Peters, and A. Toschi. PRL 2013]}\\[15pt]
\begin{itemize}
\nitem shows \alert{quantitative} agreement with NRG-DMFT (crossover scale and kinks in self-energy)\\[15pt]
\nitem Suggests that the minimal model can capture \alert{spin susceptibility}\\[10pt]
\begin{center}
\includegraphics[width=0.6\textwidth]{spin-sus.pdf}\\[15pt]
\end{center}
\nitem Our \(J-U_b\) model \alert{goes further} by capturing physics beyond the transition
\end{itemize}
}
\only<2>{
\textbf{\alert{Doublon-holon correlators} of the Hubbard model}\\
\small{\it [S. B. Lee, J. v Delft, and A. Weichselbaum. PRL 2017]}\\[5pt]
\begin{minipage}{0.3\textwidth}
Lee et. al show \alert{peaks} in \\
doublon-holon correlators\\
near zero energy within\\
the central Kondo peak.
\end{minipage}
\hspace*{\fill}
\begin{minipage}{0.65\textwidth}
\begin{center}
...
\begin{frame}{Effect on the local Fermi liquid}
\footcite{held_2013}
\textbf{Vanishing of the \alert{Kondo scale} \(T_K\) towards the transition}
\vspace*{\fill}

\begin{itemize}
\only<1>{
	\begin{minipage}{0.4\textwidth}
		\nitem Consider the \(J-U_b\) model, but with a \alert{Lorentzian DOS} in the bath:\\[5pt]
	\[\rho = \rho_0 \frac{\Gamma^2}{D^2 + \Gamma^2}\]
	\end{minipage}
	\hspace*{\fill}
	\begin{minipage}{0.3\textwidth}
	\begin{center}
	\includegraphics[width=\textwidth]{lorentzian.pdf}
	\end{center}
	\end{minipage}\\[20pt]
	\nitem Near the transition \(r = -U_b/J_0 \to \frac{1}{4}\) and \(\Gamma \to 0\), the \alert{IR energy scale} \(D^*\) can be approximated as
	\[D^* = D_0 \exp \left[\frac{\left( 2\omega + U_b + J_0/2 \right)^2}{8|U_b|\rho_0 \Gamma^2}\ln |1 - 4r|\right], ~ ~ D_0 = \text{UV cutoff}~.\]
}
\only<2>{
	\nitem \alert{Kondo temperature} can be defined as \(T_K = D^*/k_B\):
	\[T_K = \frac{D_0}{k_B} \exp \left[\frac{\left( 2\omega + U_b + J_0/2 \right)^2}{8|U_b|\rho_0 \Gamma^2}\ln |1 - 4r|\right]\]\\[10pt]
	\begin{minipage}{0.45\textwidth}
...
\begin{frame}{Effective Hamiltonian at the critical point}
	\textbf{What is the nature of the low-energy excitations \alert{exactly at the critical point}?}
	\vspace*{\fill}

\only<1-2>{
\begin{itemize}
\only<1>{
	\nitem Singlets become \alert{degenerate} with local moment states at the critical point:
	\[\frac{1}{\sqrt 2}\left(\ket{\uparrow,\downarrow} - \ket{\downarrow,\uparrow}\right), ~ ~ \ket{\sigma}\ket{0}, ~ ~\ket{\sigma}\ket{2}\]\\[20pt]
}
\nitem \alert{Non-zero degeneracy} exists even in presence of hopping perturbation:
\begin{minipage}{0.4\textwidth}
	\[\ket{N_\text{tot}=2,~ S^z_\text{tot}=0},~~\ket{N_\text{tot}=3,~ S^z_\text{tot}=\pm 1/2}~,\]
	\[\ket{N_\text{tot}=4,~ S^z_\text{tot}=0}\]\\
	\[N_\text{tot} = \hat n_d + \hat n_0 + \hat n_1, ~ ~ ~ S^z_\text{tot} = S^z_d + S^z_0 + S^z_1\]\\[10pt]
\end{minipage}
\hspace*{\fill}
\begin{minipage}{0.5\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{perturbation2.pdf}
\end{center}
\end{minipage}
\only<2>{
	\nitem Presence of magnetic states with \(S^z_\text{tot} \neq 0\) indicates \alert{breakdown of screening}\\[20pt]
	\nitem Non-vanishing degeneracy will lead to critical correlations and \alert{non-Fermi} liquid physics~\footcite{varma2002singular}
}
\end{itemize}
}
\only<3-5>{
\begin{itemize}
	\nitem \(S^z_{\text{tot}} = 0\) first order effective Hamiltonian acquires quasi non-local terms:
	\[H_\text{eff} = t\vec{S}_d\cdot\left(\vec{\mathcal{S}}^-_1 + \vec{\mathcal{S}}^-_3 - \vec{\mathcal{S}}^+_1 - \vec{\mathcal{S}}^+_3\right) + \frac{t}{4}\mathcal{P}_{01} + H_\text{K.E.}\]\\[20pt]
\only<3>{\centering
	\includegraphics[width=0.9\textwidth]{spin-defs.pdf}
}
\only<4>{
	\begin{minipage}{0.4\textwidth}
		\nitem Ground-state dictated by \(\mathcal{S}^-_1,\mathcal{S}^-_3\):\\ degenerate singlets\\[20pt]
	\nitem Spreading of extent of bath spin indicates \alert{stretching} of singlet in real space
	\end{minipage}
	\hspace*{\fill}
	\begin{minipage}{0.45\textwidth}
	\includegraphics[width=\textwidth]{stretched.pdf}
	\end{minipage}
}
\only<5>{
	\nitem Shows the \alert{dispersion} of entanglement across the lattice at critical point. Precursor to decoupling of impurity spin.\\[30pt]
	\nitem Degenerate singlets connected by hopping; reminiscent of \alert{multichannel} Kondo physics and source of non-Fermi liquid physics
}
\end{itemize}
}
...
\begin{itemize}
\only<1>{
	\nitem The extended SIAM appears to capture the phenomenology of the DMFT transition and \alert{self-consistency}.\\[30pt]
	\nitem The key ingredient is a \alert{competition} between Kondo screening physics and a local attractive correlation in the bath.\\[30pt]
	\nitem Crucial feature of the journey is the enhancement of \alert{pairing fluctuations} in the bath: leads to destruction of Kondo cloud.
}
\only<2>{
	\nitem An \alert{emergent self-consistency} is achieved through the qualitative similarity of the spectral functions of the impurity and zeroth sites.\\[30pt]
	\nitem Approach towards criticality is marked by \alert{vanishing} metallic energy scale and quasiparticle residue.\\[30pt]
	\nitem \alert{Non-Fermi liquid} physics arises at critical point, through the expansion of the singlet into the next lattice site.
}
\end{itemize}	
\end{frame}
...
\only<1>{
	\begin{minipage}{0.45\textwidth}
	\nitem The extended SIAM can be improved by considering \alert{multiple impurities} and general impurity \alert{filling}.\\[10pt]
	\nitem We are developing a new \alert{tiling-based auxiliary model method} can used for studying other models of strong-correlations as well as topologically active or flat band systems.
	\end{minipage}
	\hspace*{\fill}
	\begin{minipage}{0.4\textwidth}
	\includegraphics[width=\textwidth]{gen-phase-diagram.pdf}
	\end{minipage}\\[20pt]
...
\begin{frame}{}
\section{More insight into self-consistency}
\end{frame}

\begin{frame}{More insight into self-consistency}

{\bf Meaning of exact self-consistency}

\vspace*{\fill}

\begin{itemize}
\only<1>{
	\nitem Minimal impurity model \(\implies\) qualitative self-consistency \(\implies\) impurity site \(=\) zeroth site\\[10pt]
	\nitem Full self-consistency \(\implies\) equivalence of self-energy on all sites

\vspace*{\fill}

}
\only<2>{
	\nitem Can be achieved through an extended impurity model with \(U_b\) and \(J\) on all sites in the bath
	\vspace*{\fill}
	\begin{center}
		\includegraphics[width=0.9\textwidth]{QImp.pdf}
	\end{center}
}
\end{itemize}
