...
\begin{itemize}
	...
	\nitem Near the transition \(r = -U_b/J_0 \to \frac{1}{4}\) and \(\Gamma \to 0\), the fixed-point momentum energy scale \(D^*\) can be approximated as
	\[D^* = D_0 \exp \left[\frac{\left( 2\omega + U_b + J_0/2 \right)^2}{8U_b\rho_0 \Gamma^2}\ln |1 - 4r|\right], ~ ~ D_0 = \text{UV cutoff}~.\]\\[5pt]
	\nitem Kondo temperature can be defined as \(T_K = D^*/k_B\), vanishes towards the critical point
...
\only<1>{
\begin{itemize}
	\nitem Singlets become degenerate with local moment states at the critical point - \alert{degenerate perturbation theory}.\\[20pt]
	\nitem Hopping perturbation reduces degeneracy to \(4-\)fold:
	\[(N_\text{tot}, S^z_\text{tot}) = (2,0), ~ (3, 1/2), ~ (3,-1/2),~ (4, 0)\]\\[10pt]
...
\begin{itemize}
	...
	\[H_\text{eff} = t\vec{S}_d\cdot\left(\vec{\mathcal{S}}^-_1 + \vec{\mathcal{S}}^-_3 - \vec{\mathcal{S}}^+_1 - \vec{\mathcal{S}}^+_3\right) + \frac{t}{4}\mathcal{P}_{01} + H_\text{K.E.}\]
\only<2>{\centering
	\includegraphics[width=0.9\textwidth]{spin-defs.pdf}
}
\only<3>{
	\nitem Spreading of bath spin indicates \alert{stretching} of singlet in real space\\[10pt]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{stretched.pdf}\\[10pt]
	\end{center}
	\nitem Shows the \alert{dispersion} of entanglement across the lattice at critical point. Precursor to decoupling of impurity spin.
}
\end{itemize}
}
