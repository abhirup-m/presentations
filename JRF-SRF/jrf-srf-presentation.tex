...
\begin{itemize}
	...
	\[\Delta J = -J(J + 4U_b)n(D)\frac{1}{\omega - D/2 + U_b/2 + J/4}\]\\[5pt]
	\nitem Numerator structure allows \alert{averting} strong-coupling behaviour\\[10pt]
\end{itemize}
...
\begin{itemize}
	...
	\[\Delta J = -\frac{(J + 2U_b)^2 n(D)}{\omega - D/2 + U_b/2 + J/4} + \frac{(2U_b)^2 n(D)}{\omega - D/2 + U_b/2 + J/4}\]
\end{itemize}
...
\centering
\underline{The Kondo cloud \alert{weakens, and is destroyed} at the transition.}
...
\centering
\underline{\alert{Subdominant pairing fluctuations}, near the transition...}
...
\underline{Suggests that \alert{$J$ and $U_b$ are the minimal \& universal ingredients} for transition!}
...
\only<2>{
\vspace*{\fill}
\underline{Essence of \alert{self-consistency}: Equivalence of impurity and zeroth sites!}
}
\end{frame}
...
\only<2->{
\underline{\alert{Can be explained} heuristically using the two site spectrum}\\[10pt]
\vspace*{\fill}
\begin{itemize}
...
\only<1>{
\underline{\alert{Poor Man's scaling} of the effective Kondo model}\\
\small{\it [K. Held, R. Peters, and A. Toschi. PRL 2013]}\\[10pt]
\begin{itemize}
...
\only<2>{
\underline{\alert{Doublon-holon correlators} of the Hubbard model}\\
...
\begin{frame}{Effect on the local Fermi liquid}
	\underline{What about the \alert{low-energy excitations} of the bath, that lie above the singlet ground state?}
\begin{itemize}
...
\begin{frame}{Effect on the local Fermi liquid}
\footcite{held_2013}
\underline{Vanishing of the \alert{Kondo scale} \(T_K\) towards the transition}
\vspace*{\fill}
...
\begin{frame}{Effect on the local Fermi liquid}
	\underline{How do the imaginary part of \alert{self-energy} \(\Sigma\) and the \alert{quasiparticle residue} behave near the transition?}
	\vspace*{\fill}
